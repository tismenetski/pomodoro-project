...
def reset_timer():
    ...
    global timer
    screen.after_cancel(timer) # Cancel a timer in our program.
    #screen.destroy()
    #count_down(0)
...
def count_down(count):
    ...
    if count > 0:
        ...
        if reps % 2 == 0:
            #text_label = check_mark_label.cget()
            check_mark_label.config(text=check_mark_label.cget('text')+CHECK_MARK)
        start_timer()
...
check_mark_label = Label(font=(FONT_NAME,12,"bold"),bg=YELLOW,fg=GREEN)
